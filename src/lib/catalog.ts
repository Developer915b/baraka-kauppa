// Static product catalog — single source of truth shared with the DB seeder
// (scripts/seed-products.ts). Used as a resilient fallback by the API routes
// when no database is available (e.g. serverless hosting on Netlify), and the
// DB seeder imports the same data so ids stay in sync (1..N in array order).
...
/** Replicates the DB query semantics of GET /api/products for the static catalog. */
export function filterCatalog(opts: { category?: string | null; q?: string | null }): CatalogProduct[] {
  let list = CATALOG.filter((p) => p.stock > 0);
  ...
  if (q) {
    list = list.filter(
      (p) =>
        ...
        p.descFi.toLowerCase().includes(q)
    );
  }

  return [...list].sort((a, b) => a.category.localeCompare(b.category) || a.price - b.price);
}
